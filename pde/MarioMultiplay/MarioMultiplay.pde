...
import org.yaml.snakeyaml.Yaml;
	...
	int MY_MARIO_NUM;
	String ROOM_NUM;
	float MY_FRICTIONAL_FORCE = 0.4f;  // 정상 0.4.   얼음 세상: 0.1
	...
	public void doNetworkStuff() {
		networkWrite();  // 먼저 조립된 내 마리오 상태 보내고
		networkRead();  // 고스트 마리오를 세팅하자
	}
	...
	class Mario {
		...
		public void _updatePosition() {
			...
			if (! isGhost) {  // FTS: Favor The Shooter, 고스트는 밟기 판정을 안함
			...
			// Camera following
			if (getCenterX() - camera.getCenterX() > 0) {  // 마리오 너무 오른 쪽
				camera.x = getCenterX() - 0.5f * camera.width;
			} else if (camera.getCenterX() - getCenterX() > CAMERA_RANGE_LEFT) {  // 마리오 너무 왼쪽
	...
	class CollisionDetector {
		public boolean up(Mario mario) {
			...
			for (int i = 0; i < 2; ++i) {
				...
				if (blockRow.size() <= bX) continue;  // 블록이 없네
...
//			String channel = (ROOM_NUM + "CHANNEL_MARIO_0");
		...
		public void run() {
			while (!Thread.currentThread().isInterrupted()) {
				/* 동기적으로 읽기를 해서, sleep 안해줘도 된다. 샘플코드에서 따온 코드임
				 */
